...
exports.getVoucherRequests = async (req, res) => {
  try {
    const { data, error } = await supabase.from("voucher_requests").select("*");
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch voucher requests",
      error: error.message,
    });
  }
};
    ...
    if (status === "approved") {
      ...
      if (updateError) {
        ...
        return res.status(500).json({
          success: false,
          message: "Failed to update voucher balance",
...
exports.updateItemInventory = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Item inventory updated successfully" });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Failed to update inventory", error });
  }
};
...
exports.addNewUser = async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert([{ email, phone, name, data: userData }]); // Insert a new user record
